Function ConvertTo-SQLWhereString {
	<#
.SYNOPSIS
		...
		If ($Parameters) {
			...
			$Parameters.Keys | ForEach-Object {
				...
				If ($NoEscape) {
					...
					if(-not[string]::IsNullOrEmpty($Parameters[$PSItem]) -And $Parameters[$PSItem].GetType() -eq [string] -and  $Parameters[$PSItem].Contains("%")){
						$Value = "$PSItem like'$($Parameters[$PSItem])'"
