...
Function Connect-OIMSQL($servername, $database, [PSCredential] $Cred ) {
    ...
    $OIM_SQLConnection = New-Object System.Data.SQLClient.SQLConnection
    $OIM_SQLConnection.ConnectionString = $ConnectionString
    $OIM_SQLConnection.Open()

    Set-Variable -scope Global -name OIM_SQLConnection -Value $OIM_SQLConnection
    $OIM_SQLConnection
}
...
function Invoke-OIMSQLQuery {
    ...
    param(
        ...
        [string]
        $sqlquery,
        $connection = (Get-Variable -scope Global -name OIM_SQLConnection)
